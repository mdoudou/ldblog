...
func SearchListPublishedPost(tag string, pageIndex, pageSize int, searchname string) ([]*Post, error) {
	return _searchlistPost(tag, true, pageIndex, pageSize, searchname)
}
func ListAllPost(tag string) ([]*Post, error) {
	return _listPost(tag, false, 0, 0)
}
func _searchlistPost(tag string, published bool, pageIndex, pageSize int, searchname string) ([]*Post, error) {
	var posts []*Post
	var err error
	if len(tag) > 0 {
		...
		if published {
			if pageIndex > 0 {
				rows, err = DB.Raw("select p.* from posts p inner join post_tags pt on p.id = pt.post_id where pt.tag_id = ? and p.is_published = ? and p.title like ? or p.body like ? order by created_at desc limit ? offset ?", tagId, true, "%"+searchname+"%", "%"+searchname+"%", pageSize, (pageIndex-1)*pageSize).Rows()
			} else {
				rows, err = DB.Raw("select p.* from posts p inner join post_tags pt on p.id = pt.post_id where pt.tag_id = ? and p.is_published = ?  and p.title like ? or p.body like ? order by created_at desc", tagId, true, "%"+searchname+"%", "%"+searchname+"%").Rows()
			}
		} else {
			rows, err = DB.Raw("select p.* from posts p inner join post_tags pt on p.id = pt.post_id where pt.tag_id = ? and p.title like ? or p.body like ? order by created_at desc", tagId, "%"+searchname+"%", "%"+searchname+"%").Rows()
		}
		...
		if published {
			if pageIndex > 0 {
				err = DB.Where("is_published = ? AND title LIKE ? or body LIKE ?", true, "%"+searchname+"%", "%"+searchname+"%").Order("created_at desc").Limit(pageSize).Offset((pageIndex - 1) * pageSize).Find(&posts).Error
			} else {
				err = DB.Where("is_published = ? AND title LIKE ? or body LIKE ?", true, "%"+searchname+"%", "%"+searchname+"%").Order("created_at desc").Find(&posts).Error
			}
		} else {
